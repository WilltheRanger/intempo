...
describe('with a piece to continue', () => {
  ...
  it('says nothing about the last take', () => {
    // The owner cut the verdict line (2026-09-23): it is the Verdict screen's
    // to say, and it arrived after the rest of the hero had drawn.
    expect(heroContentFor({ piece: piece(), workingBpm: 92 }).detail).toBeNull();
  });

  it('says the notation is coming while it is still being read', () => {
    // Silence would leave a musician looking at a title and a button with no
    // clue the app is mid-way through something.
  ...
  it('promises the screen the tap actually opens', () => {
    ...
    expect(
      heroContentFor({ piece: piece({ score: SCORE(0) }), workingBpm: 92 }).actionLabel,
    ).toBe('Open');

    expect(heroContentFor({ piece: piece(), workingBpm: 92 }).actionLabel).toBe('Practice');
  });
...
describe('with nothing in the library', () => {
  ...
  it('offers no metadata rather than an empty line of it', () => {
    expect(heroContentFor({ piece: null, workingBpm: 0 }).meta).toBeNull();
  });
});
